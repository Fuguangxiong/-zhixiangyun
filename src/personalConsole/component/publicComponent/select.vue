<template>
  <div class="msginput" @click.stop="toggle" :class="{active: showSelectlist, edit: entManage}">
    <i class="iconfont icon-icon-up"></i>
    <input disabled :placeholder="preview" v-model="curdata" type="text" :class="{'border_error':verify,'border_width':industryVerify,'inputBorder':borderNum}">
    <div class="selectlist" v-show="showSelectlist" ref="main" :class="{'border_width':industryVerify}">
...
<script>
  export default {
    name: 'myselect',
    props: ['data', 'preview', 'curvalue', 'verify', 'entManage','industryVerify','borderNum'],
    mounted () {
    },
    watch: {
      curvalue (val, oldval) {
