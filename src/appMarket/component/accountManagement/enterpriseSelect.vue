...
import topComponent from './publicTop'
    ...
    init () {
      ...
      ApiClient.get('pubapi/user/getLoginInfo').then(data => {
        ...
        if (e.response.status === 401 || e.response.status === 403) {
          window.location.replace(publicPathConfig.platformLoginUrl)
        } else {
          console.error(e)
        }
      })
