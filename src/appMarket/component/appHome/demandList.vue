...
import commonFunc from "../../../serviceManage/component/common/commonFunc.js";
...
import publicPathConfig from '../../../publicJs/publicPathConfig.js'
  export default{
    ...
    //服务详情
     shenqingdetail() {
      let vm = this;
      let param = { id: this.serviceData.id };
    ...
    onSubmit() {
      ...
      apiClient.post(vm.url.submit, param, function(data) {
        ...
        if (data.status == 200) {
          ...
          window.location.href=publicPathConfig.PERSONAL_CONSOLE+'#/applied' 
        } else {
          TipBoxService.open(data.exception, 2);
        }
      });
