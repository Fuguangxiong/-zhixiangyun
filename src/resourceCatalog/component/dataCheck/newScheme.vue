    ...
    <div class="resou-con">
      <div class="w1200 h100">
        ...
        <div class="mode-div lanm-con">
          ...
          <div class="scheme_main" v-show="isShowFirstStep">
            ...
            <div class="scheme_center">
              ...
              <div class="scheme_described clearfix">
                ...
                <textarea class="texe_sd" name="description" v-model="dataCheckSpecs.description"></textarea>
              </div>
            </div>
            <div class="buttonBox">
          ...
          <LabelView :dialogTagVisible.sync="dialogTagVisible" isCreateModel="true" :model="dataCheckSpecs" modelLabelKey="labels" />
          <!-- 第一步 end -->
          ...
          <div class="scheme_main" v-show="isShowSecondStep">
            ...
            <div class="scheme_center">
              ...
              <div class="detection_type">
                <div class="detection_title"><span class="red-star">*</span>资源类别</div>
                <div class="radic_xz">
                  <span class="radic_ico aclick">
                    <i></i>
                  </span>
              ...
              <div class="detection_type_sele" v-show="!resourceName">
                <div class="detection_title"><span class="red-star">*</span>资源位置</div>
                <!-- <span class="popupBoxLineTitle">选择资源</span> -->
                <el-cascader
                  expand-trigger="click"
                  :options="resourceLocation.selectData"
                  :props="resourceLocation.props"
                  v-model="resourceLocationValue"
                  change-on-select
                  :show-all-levels="false"
                  @change="handleChange" style="left:15px">
                </el-cascader>
              </div>
              <div class="detection_type_sele" v-show="!resourceName">
                <div class="detection_title"><span class="red-star">*</span>资源项</div>
                <el-select v-model="resourceItemValue" filterable placeholder="请选择" 
                @change="itemHandleChange" style="left:15px">
                  <el-option
                    v-for="item in resourceItem"
                    :key="item.id"
                    :label="item.name"
                    :value="item.id">
                  </el-option>
              ...
              <div class="detection_type_sele" v-show="resourceName">
                <div class="detection_title"><span class="red-star">*</span>资源项</div>
                <el-select v-model="resourceName" disabled placeholder="请选择" style="left:15px">
                  <el-option
                    v-for="item in resourceItem"
                    :key="item.id"
                    :label="item.name"
          ...
          <div class="scheme_main" v-show="isShowThirdStep">
                ...
                <div class="ms_main">
                  <table width="100%" class="ms_tab">
                    ...
                    <tbody>
                      ...
                      <tr v-for="(targetItem, index) in dataCheckSpecs.targetList" :key="targetItem">
                        ...
                        <td v-if="targetItem.metadataCodesLength === 0" style="text-align: center">-</td>
                        <td v-else style="text-align:center" class="m_color xz_data" @click="checkMetadataCodes(index)">{{targetItem.metadataCodesLength + '项'}}</td>
                        <td style="text-align: center" class="ms_bgcolor">
                          <select @change="fieldHandleChange(targetItem, $event)" class="ms_select">
                            <option value="" disabled :selected="!targetItem.targetDesc">请选择</option>
                            <option v-for="item in detectionField" :value="item.name" :key="item.name">
                              {{item.desc}}
                            </option>
                          </select>
                        </td>
            ...
            <el-dialog title="选择数据元" :visible.sync="dialogTableVisible" class="numClass">
              <div class="numClassone">
                <span >数据元名称</span>
              ...
              <div class="detection_type_sele">
                ...
                <div class="threshold_main">
                  <div class="threshold_left">
                    ...
                    <div class="tl_select">
                      <input type="text" v-model="dataCheckSpecs.value2FullyUsableThreshold" 
                      :class='{wrong:dataCheckSpecs.value2FullyUsableThreshold == "" && isFourthSubmit}' class="inp_tl"/>
                    </div>
                  </div>
                </div>
              </div>
              <div class="detection_type_sele">
                ...
                <div class="threshold_main">
                  <div class="threshold_left">
                    ...
                    <div class="tl_select">
                      <input type="text" v-model="dataCheckSpecs.value2BarelyUsableThreshold" 
                      :class='{wrong:dataCheckSpecs.value2BarelyUsableThreshold == "" && isFourthSubmit}' class="inp_tl"/>
                    </div>
                  </div>
                </div>
              </div>
              <div class="detection_type_sele">
                ...
                <div class="threshold_main">
                  <div class="threshold_left">
                    ...
                    <div class="tl_select">
                      <input type="text" v-model="dataCheckSpecs.value2BarelyUsableThreshold" disabled class="inp_tl"/>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="scheme_main" v-show="isShowFifthStep">
            ...
            <div class="scheme_center">
              <div class="wanc">
                ...
                <span class="gongxi">恭喜您，您的检测方案配置完成！</span>
              </div>
            </div>
...
export default {
  ...
  mounted () {
    ...
    if (this.$route.query.resourceName) {
      this.findResourcesById(this.$route.query.resourceId)
    }
  ...
  data () {
    return {
      dataCheckSpecs: { // 检测方案
        name: '',
        resourceId: '', // 资源项ID
        targetList: [], // 数据检测指标（对应的目标字段与数据元）
        value2FullyUsableThreshold: '', // 完全可用阈值
        value2BarelyUsableThreshold: '', // 一般可用阈值
        labels: []
      },
  ...
  methods: {
    ...
    toThirdStep () {
      ...
      // console.log('新建方案第二步：', this.dataCheckSpecs)
      this.isShowFirstStep = false
      this.isShowSecondStep = false
    ...
    getResourceLocation () {
      apiClient.get('/rcapi/catalog/getTab?type=data').then(data => {
        let resData = UtilService.filterChildren(data)
        this.resourceLocation.selectData = resData
      }).catch(e => {})
    },
    handleChange (data) {
      this.resourceItem.length = 0
      this.resourceItemValue = ''
      this.dataCheckSpecs.resourceId = ''
      if (data.length < 2) {
        return
      }
      ...
      apiClient.get('/rcapi/findResourceItems', params).then(data => {
        LoadingBoxService.close()
        this.resourceItem = data.dataList
      }).catch(e => {
        LoadingBoxService.close()
      })
    },
    itemHandleChange (data) {
      this.dataCheckSpecs.resourceId = data
      apiClient.get('/rcapi/findResourcesById', {resourceId: data}).then(data => {
        let dbfields = []
    ...
    findResourcesById (respurceId) {
      this.dataCheckSpecs.resourceId = respurceId
    ...
    saveMetadata () {
      // this.dataCheckSpecs.targetList[this.targetListIndex] = this.newTargetList
      this.dataCheckSpecs.targetList[this.targetListIndex].metadata = this.newTargetList.metadata
      this.dataCheckSpecs.targetList[this.targetListIndex].metadataCodesLength = this.newTargetList.metadataCodesLength
    ...
    toFifthStep () {
      let threshold1 = this.dataCheckSpecs.value2FullyUsableThreshold
      let threshold2 = this.dataCheckSpecs.value2BarelyUsableThreshold
      ...
      this.dataCheckSpecs.resourceMode = 'SINGLE_MODE'
      this.dataCheckSpecs.resourceLocation = 'PLATFORM'
      this.dataCheckSpecs.usable = true
      this.dataCheckSpecs.id = util.getId()
      // console.log('新建方案第四步：', this.dataCheckSpecs)
      apiClient.post('/rcapi/dataCheckSpecs/save', this.dataCheckSpecs).then(data => {
        this.isShowFirstStep = false
        this.isShowSecondStep = false
        this.isShowThirdStep = false
        this.isShowFourthStep = false
    ...
    checkName () {
      ...
      if (this.dataCheckSpecs.name.trim() !== '') {
...
<style scoped>
  ...
  .dialog-footer button:last-child {
   ...
   background-color:#27D188 !important;
  }
