...
export default {
    ...
    case ('5'): // 我的注册
      if (obj.audit_status == '2' && obj.service_status == '1' && obj.up_down_status == '1') {
        ...
        btns = ['delete', 'viewApplyRegist']
          ...
          btns = ['revokeRegist', 'viewApplyRegist']
        } else if (obj.audit_status == '3') {
          if (obj.service_status == '6') {
            btns = ['viewReason', 'viewApplyRegist']
          } else {
  ...
  toEdit: function (Vue, item) {
    // 企业服务、已注册服务编辑服务
    if (item.sourceType == '1' || item.sourceType == '5') {
      if (item.isatomic == 'false') {
        ...
        Vue.$router.push({
          ...
          query: { atomicId: item.id, source_type: Vue.sourceType }
        })
      }
    }
    // 已申请服务编辑申请
  ...
  toModify: function (Vue, item) {
    ...
    /* let params = { "serviceId": item.id }; //传入要变更的
     apiClient.get('itsmApi/atomicService/copyService', params, function(data) {
       if (data.status == "200") {
         //跳转到修改页面路由
         Vue.$router.push({
           name: "newServicePage",
           query: { id: data.results.id, routerName: "toModify" }
         });
       } else {
  ...
  toDetailPage: function (item, btnArray) {
    ...
    if (item.isatomic == 'false') {
      window.open('#/servicePortfolioDetails?id=' + item.id + '&btnKey=' + key_btn, '_blank')
    } else {
      window.open(
        '#/serviceInfo?id=' + item.id + '&audit_status=' + item.audit_status + '&btnKey=' + key_btn,
        '_blank'
      )
    }
  },
