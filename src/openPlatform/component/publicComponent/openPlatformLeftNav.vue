<template>
  <div class="leftTotalContainer">
    <div class="leftContainer">
       <div class="leftNavName">开发者平台</div>
      <div class="leftBottomContainer">
        <div class="layoutLeftBigContainer">
          <div class="layoutLeftContainer">
            <div v-for="(item,index) in navArr" :key="index" class="layoutLeftNavItem" :title="item.name" v-bind:class="{select:item.active}" v-on:click="navClickHandler(item, index)">
              <i class="leftNavImage" v-bind:class="item.imageClass"></i>
              <span>{{item.name}}</span>
            </div>
          </div>
    ...
    <div class="leftChildrenContainer" :class="{childrenHide:childrenHide}">
      <!--<div class="leftChildrenHeader"></div>-->
      <div class="leftChildrenTitle">{{childrenTitle}}</div>
      <div class="leftChildrenItemContainer">
        <div v-for="(val,index2) in childrenArr" :key="index2" :class="{active:val.active}" class="leftChildrenItem" v-on:click="childNavClickHandler(val)">{{val.name}}</div>
      </div>
    </div>
  </div>
</template>
