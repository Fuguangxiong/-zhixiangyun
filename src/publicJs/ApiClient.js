...
function ApiClient () {
  ...
  this.callApi = function (method, callBack) {
    ...
    return axios(this.config).then(response => {
      ...
      if (error.response && error.response.status === 401) {
        ...
        // console.info(href.indexOf('register'))
        if (href.indexOf('accountRegister') < 0) {
  ...
  this.uploadMultiFiles = function (fileAndNames, data, args) {
    ...
    var _token = data.token || this.config.data.token
    if (!_token) {
    ...
    let config = {
      ...
      headers: {
        ...
        'X-Auth-Token': this.config.headers['X-Auth-Token']
      }
    }
    let callBack = getCallBack(args)
  ...
  this.identityAuthentication = function () {
    let logintype = userService.getInfo('logintype') || 'undefined'

    if (logintype === 'undefined') {
      return
    } else if (logintype === 'PLATFORM') {
      window.location.href = Constants.internalLoginUrl
    } else {
      userService.deleInfo('tokenId')
      userService.deleInfo('logintype')
      userService.deleInfo('username')
      window.location.href = Constants.appMarket
    }
  }
