...
let commonProblem = ''
let introduceServer = ''
let appName = process.envConfig.APPNAME
let logout = ''
$.getJSON(process.envConfig.EXTERNAL_CONFIG_URL, function (data) {
  ...
  logout = data.PLATFORM_LOGOUT_URL
  platformUser = data.PLATFORM_USER
  platform = data.PLATFORM
  internalOperation = data.INTERNAL_OPERATION
  companyOperation = data.COMPANY_OPERATION

  securityManage = data[appName].SECURITY_MANAGE
  basicInfo = data[appName].BASIC_INFO
  realName = data[appName].REAL_NAME_AUTHENTICATION
  createEnter = data[appName].CREATE_ENTERPRISE
  joinEnter = data[appName].JOIN_ENTERPRISE
  identityManage = data[appName].IDENTITY_MANAGE
  ...
  openPlatform = data.OPENPLATFORM
  opApiPath = data.OPEN_PLATFORM_API_PAHT
...
export default {
  ...
  CREATE_ENTERPRISE: createEnter, // 创建企业
  JOIN_ENTERPRISE: joinEnter, // 加入企业
