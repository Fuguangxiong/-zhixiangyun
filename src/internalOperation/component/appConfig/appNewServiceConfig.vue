...
<div class="appImgConfigBox appdialog">
	<div class="appHotConfigTop">
			<div class="appHotConfigTopLeft">
				<span >新锐应用(最多可新建30条)</span><em class="maxsize"></em>
			</div>
			<div class="appHotConfigTopRight" @click="selectAll">新增</div>
		</div>
		<!--新增新锐应用弹框-->
		<el-dialog title="新锐应用新建" :visible.sync="dialogTableVisible">
		      <div class="modal-body">		     
		         <div class="appHotConfigTcTop">
		         	<span class="icoLn"></span><em>新增新锐应用</em>
		         </div>
		          <div class="appHotConfigTcIp">		          		
					 	<div class="aui" @click.self="isShowSearched=false">
							<input id="label-input" name="label-input" v-model.trim="inputValue" class="text" autocomplete="off" placeholder="请输入内容">
						</div>
		...
		<div class="appHotConfigContent">
			<div v-if="notNull" class="conLst">
			 <!--拖拽-->
		      <draggable class="conoxList" v-model="serverArray" :move="getdata" @update="datadragEnd">
		      	
				 <div  class="serverli" v-for="(list,index) in serverArray" :key="list.id">
					...
					<div class="appHotConfigDlie" >
							<i @click="deleteNewServer(list)"></i>
					</div>
...
export default {
  ...
  mounted () {
    this.getNewServiceInfo()
    var that = this
    window.addEventListener('click', function (e) {
      if (e.target._prevClass == 'icon iconfont icon-gengduo') {
        return
      }
      that.a = -1
    })
  ...
  methods: {
    remoteMethod (value) {
      if (value !== '') {
        apiClient
          .get(apiToken.newApi('/appmarket/atomic/getAtomicFuzzySearch/'), {
            title: value
          })
          .then(data => {
    ...
    datadragEnd (evt) {
      ...
      // console.info("------------",appPublicJs.dragData(this.serverArray));
      apiClient.put(
        apiToken.newApi('appmarket/content/dragAndContentDrop/'),
        appPublicJs.dragData(this.serverArray),
        function (data) {
    ...
    getNewServiceInfo () {
      ...
      var myData = {
        ...
        contentType: 'newService',
        contentName: '新锐应用',
        pageNum: that.pageNum,
        pageSize: that.pageSize
      }
      apiClient.get(
        apiToken.newApi('/appmarket/content/getCotentByNotice/'),
        myData,
        function (data) {
          if (appPublicJs.checkData(data.results.data)) {
            ...
            if (
              data.results.total < that.serverArray.length ||
              data.results.total == that.serverArray.length ||
              that.serverArray.length == 0
            ) {
              that.ifMore = false
            } else {
              that.ifMore = true
    ...
    selectItemInList (id, name) {
      ...
      if (appPublicJs.checkData(that.serverArray2)) {
        if (this.haveItem(that.serverArray1, that.serverArray2, serverObj)) {
          ...
          that.serverArray2.unshift(serverObj) // 回显
        }
      } else {
          ...
          for (var i = 0; i < that.serverArray1.length; i++) {
            var myData = {
              ...
              sort_order: 1
            	...
            	LoadingBoxService.open('加载中...')
            apiClient.post(
              apiToken.newApi('/appmarket/content/saveCotent/'),myData, function (data) {
                	LoadingBoxService.close();
    ...
    deleteNewServer (list) {
      ...
      NoticeBoxService.open(
        ...
        function (val) {
          ...
          apiClient.delete(
            apiToken.newApi('/appmarket/content/deleteContent/'),
            myData,
            function (data) {
              if (data.results.status == true) {
        ...
        function (val) {
          ...
          if (appPublicJs.checkData(that.serverArray2)) {
            for (var i = 0; i < that.serverArray2.length; i++) {
              ...
              if (
                id2Array.indexOf(list.serverId) != -1 &&
                id1Array.indexOf(list.serverId) == -1
              ) {
                apiClient.delete(
                  apiToken.newApi('/appmarket/content/deleteContent/'),
                  myData,
...
<style src='../../css/appNewServiceConfig.css' scoped></style>
