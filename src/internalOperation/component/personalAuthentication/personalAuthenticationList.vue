<template class="public">
  <div class="personalAuthentication">
	...
	<div class="searchBoxpersonal">
	  用户名称: <input type="text" class="inputClass" placeholder="请输入申请人的姓名" v-model="content" @keyup.enter="searchApply"><span @click="searchApply"><i class="iconfont icon-sousuo"></i></span>
	</div>
	<!--多选状态-->
	<div class="publicStatus">
	  <div class="publicStatusLeft">
		<div class="wordBlock tStyel tStyelfuw" style="margin-top: 3px">
	...
	<div class="searchTime">
	  <div class="publicStatusLeft">
		<div class="wordBlock tStyel tStyelfuw" style="border-bottom: 1px solid #f1f1f1;padding-bottom: 6px;margin-top: 3px">
	  ...
	  <div v-for="(value,index) in table" :key="index" class="tableListLine">
		<div>
		  <div class="tableListLeft" style="float: left;">
			...
			<div class="time">
			  <span v-show="value.user.phone==''?false:true">联系电话：{{value.user.phone}}</span>
			</div>
			<div class="time1">
			  <span v-show="value.status=='WAITING'?false:true" class="coorperateTime">审批时间：{{value.auditTime | formatDuring}}</span>
			</div>
		  </div>
		  <div class="action">
...
export default {
  ...
  data () {
    return {
      ...
      num: -1,
      noData: true,
      breadcrumb: [{ name: '个人认证', link: '' }],
      content: '', // 搜索框内容
      datalist: [
        { name: '待审批', checkbox: false },
        { name: '已通过', checkbox: false },
        { name: '未通过', checkbox: false }
      ],
      isShow: true,
      ...
      cheboxtext: '+多选',
      timedatalist: [
        { name: '近一月', checkbox: false },
        { name: '近三月', checkbox: false },
        {
          name: '近半年',
          checkbox: false
        },
        { name: '近一年', checkbox: false },
        { name: '自定义', checkbox: false }
      ],
      // 时间选择组件
      value11: '',
      value12: '',
...
//      that.approvalStatus = ''
    ...
    toggle (e) {
      ...
      if (that.choice) {
        ...
        that.pageNum = 1
        let val =
          e === 0 ? 'WAITING' : e === 2 ? 'NO_PASS' : e === 1 ? 'PASS' : ''
    ...
    getTableData () {
      ...
      var that = this
      ApiClient.get('/pcapi/authentication/list', {
...
.personalAuthentication .searchBoxpersonal i {
...
.personalAuthentication .searchBoxpersonal {
  height: 32px;
  line-height: 32px;
  /*border-radius: 6px;*/
  margin-right: 40px;
...
.personalAuthentication .searchBoxpersonal span {
  position: absolute;
  top: 5px;
  left: 275px;
  cursor: pointer;
}
.personalAuthentication .searchBoxpersonal .button {
  display: inline-block;
...
.personalAuthentication .searchBoxpersonal .button i {
  vertical-align: top;
  margin-right: 10px;
}
.public {
...
.personalAuthentication .inputClass:hover {
...
.tableList .tableListLine > div:hover::after {
  ...
  z-index: 96;
  box-shadow: 0 0 8px rgba(85, 168, 253, 0.7);
}
.action .sliderBtn {
