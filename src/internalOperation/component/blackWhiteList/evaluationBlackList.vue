<template>
    <div class="collection">
        ...
        <div class="conCent">
            <div class="rise">
                <span class="textTlie">评价黑名单管理</span>   
            </div>
            <div class="wordBlock tStyel">
                <span class="textName">限制对象：</span>
                <div>
                    <div class="search width248">
                        <input type="text" maxlength="17" placeholder="请输入手机号搜索" class="inputStyle" v-model="myData.objName" @keyup.enter="searchEva">
                                            ...
                                            <div class="aui-dropdown aui-dropdown-left">
                                                <ol class="last">
                                                    <li  v-for="item2 in pnoneList" >
                                                        <a href="javaScript:void(0)" @click="selectItemInList(item2.ID,item2.PHONE)"  class="label-suggestion">
                                                            <span ><em>{{item2.PHONE}}({{item2.USERNAME}})</em></span>
                                                        </a>
                                                    </li>
                                                </ol>
                                            </div>
...
export default {
  ...
  mounted () {
        ...
        // this.myData.endDate=timestamp;
        // this.myData.startDate=timestamp-7* 24 * 3600 * 1000;
  ...
  methods: {
    ...
    checkIp () {
      var that = this
      var pattern = new RegExp('^(([0-2]*[0-9]+[0-9]+)\.([0-2]*[0-9]+[0-9]+)\.([0-2]*[0-9]+[0-9]+)\.([0-2]*[0-9]+[0-9]+))$')
    ...
    keyupFun (id) {
      if ($('#' + id).val().length == 0) {
        ...
        $('#type' + num).attr('disabled', false).focus()
        if (parseInt(id.replace(/[^0-9]/ig, '')) == 6) {
    ...
    submit () {
      ...
      if (that.tankuangtitle == '修改信息') {
      ...
      } else if (that.tankuangtitle == '添加用户') {
        var that = this
        var namelist = []
        ...
        apiClient.post(apiToken.newApi('/ioapi/bwlist/addBwList/'), myData).then(data => {
          if (data.status == true) {
            that.closeBtn()
            TipBoxService.open('添加评价黑名单成功', 0)
            that.myData.tagTime = '7'
            that.getList()
          } else {
            TipBoxService.open('添加评价黑名单失败', 2)
          }
        }).catch(e => {
          console.error(e)
        })
