<template>
    <div class="collection">
        ...
        <div class="conCent">
            <div class="rise">
                <span class="textTlie">黑名单管理</span>   
            </div>
            <div class="wordBlock tStyel">
                <span class="textName">限制对象：</span>
                <div>
                    <div class="search width248">
                        <input type="text" placeholder="请输入IP／MAC/手机号搜索" maxlength="17" class="inputStyle" v-model="myData.objName" @keyup.enter="searchEva">
...
export default {
  ...
  data () {
    return {
      ...
      checkMarkinfo: '', // 验证备注
      falseIp: false,
      singleServiceInp: false, // 控制服务-单个服务控制
      selectItem: false,
      checktime: '',
      showlist: true
  ...
  methods: {
    ...
    submit () {
      ...
      if (that.tankuangtitle == '修改黑名单') {
      ...
      } else if (that.tankuangtitle == '添加黑名单') {
        let that = this
        var namelist = []
        ...
        apiClient.post(apiToken.newApi('/ioapi/bwlist/addBwList/'), myData).then(data => {
          if (data.status == true) {
            LoadingBoxService.close();
            that.closeBtn()
            TipBoxService.open('添加黑名单成功', 0)
            that.myData.tagTime = '7'
            that.getList()
          } else {
            TipBoxService.open('添加黑名单失败', 2)
          }
        }).catch(e => {
          console.error(e)
        })
    ...
    radioClicks () {
      if (this.radio == 1) {
        ...
        //               var timestamp = Date.parse(new Date());
        //                this.myData.endDate=new Date(this.endvalue).getTime()
        //                this.myData.startDate=timestamp-7* 24 * 3600 * 1000;
        //           }
        //           else{
        //               this.myData.endDate=new Date(this.endvalue).getTime()
    ...
    singleServiceInput (id, name) {
      ...
      that.serverArray2.push(serverObj)
            ...
            // 用户--点击下拉框获取服务id和服务名
    selectItemInList (id, name, state) {
      if (state != 2) {
        var that = this
