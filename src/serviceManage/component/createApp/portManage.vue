<template>
  <div class="outbox" id='portManage'>
    <div class="newService">
                  ...
                  <div class="action">
                    ...
                    <div class="checkRight check control-i" style="float: right;" :class="index==num? 'checkone' : ''">
                      ...
                      <p class="applicationtime1">
                        {{item.create_time}}<br/>
                        {{item.create_time1}} {{item.create_time2}}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <div class="port-action-item">
          <i class="icon iconfont icon-iconjia" @click="onOpenAddDialog('addForm')"></i>
      </div>
    </div>
      ...
      apiClient.post(vm.url.query, param, function(data) {
        if (data.status == 200) {
          ...
          for(var i = 0 ; i < vm.tableData.length ; i ++){
            if(vm.tableData[i].create_time != undefined){
                vm.tableData[i].create_time = vm.format(dataList[i].create_time,"yyyy-MM-dd");
                vm.tableData[i].create_time1 = vm.format(dataList[i].create_time,"HH:mm:ss");
            }else{
              vm.tableData[i].create_time2 = '暂无--'
            }
          };
          LoadingBoxService.close();
