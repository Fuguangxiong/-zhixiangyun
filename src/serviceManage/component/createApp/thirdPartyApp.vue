...
export default{
  ...
  data () {
    return {
      breadcrumb: [{name: '应用管理', link: ''}, {name: '第三方应用', link: ''}],
      searchKey: {}, // 查询关键字
      selectSetting: [], // 筛选项
  ...
  methods: {
    ...
    setSelectData: function () {
      ...
      item1.data = appCommonFunc.getAppStatusDic(['全部', '待开通', '运行中'])
      let item2 = {
        label: '审核状态',
        paramName: 'audit_status',
        button: ['checkbox']
      }
      item2.data = appCommonFunc.getAuditStatusDic(['全部', '待审核', '已通过'])
    ...
    onQuery: function () {
      ...
      apiClient.post('/itsmApi/application/queryThirdPartyAppList', params, function (data) {
    ...
    onCreatTable: function (dataList) {
      ...
      dataList.forEach(function (v, i) {
        ...
        obj.watermarkClass = appCommonFunc.addWatermark(obj.audit_status)
                // 高级 按钮 功能
        obj = appCommonFunc.addSeniorBtns(obj, 'thirdParty')
                // 右侧hover功能
