...
<LabelView :dialogTagVisible.sync="dialogTagVisible" :isCreateModel="!isEdit" :model="checkedLabels" modelLabelKey="tagArray" :changeLabelCaller="saveLabel" /></div>
	</div>
</template>
<script>
	...
	export default{
		...
		methods:{
				...
				if(this.$route.query.atomicId != undefined){
					...
					apiClient.post('itsmApi/atomicService/loadServiceInfo',myData,function(data){
		    			...
		    			if(data.status == 200){
							...
							that.Param.apply_time = obj.apply_time;
							that.Param.open_time = obj.open_time;
            ...
            saveAllInfo(nameInfo){
				...
				apiClient.post('itsmApi/atomicServiceDev/updateAtomicInfo',myData,function(data){
            			if(data.status == 200){
            				if(nameInfo == 'toNext'){
            					LoadingBoxService.close();
            					that.$router.push({name: 'newServiceFinish'})
	            			}else{
	            				LoadingBoxService.close();
