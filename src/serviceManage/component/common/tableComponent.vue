  ...
  <div id="tableComponent">
    <div class="queryListWord">
      ...
      <div class="more" v-show="false">
        <div @click="" class="sure">删除</div>
        <div class="cancel" @click="">取消</div>
      </div>
    </div>
    
    <div class="listCon" :class='{"no-data":totalNumber==0}'>
      <!-- 列表 -->
      <div v-for="(item,index) in tableList" :id='item.id+item.service_id' :key="index" class="listLi" :class="{abnormalClx:true}">
        <div class="listLiLeft" :class="{scheme:item.scheme_status,'userInfo':item.sourceType =='userNumInfo'}">
          <dl>
            ...
            <dt class='watermark' :class='item.watermarkClass'>
              <h2>
                <a v-show='item.name' @click='onClickName(item)' href='#' class='listLiLeftName'>
                  {{item.name}}
                  <b v-show='item.change_id'>（变更中）</b>
                </a>
                <span v-show='!item.sourceType' class='listLiLeftName'>{{item.scheme_name}}</span>
                <span v-if='item.tag' v-for="(tag,i) in item.tag" class='statueTag' :class='tag.className'>{{tag.text}}</span>
              </h2>
              <el-row :gutter="15">
                <el-col v-for='(detail, detailIndex) in item.detail' 
                        :span="detail.width=='short'? 8 : 24"
                        :offset="detail.wrap? 3 : 0">
                  <h3>
                    {{detail.label}}：
  ...
  methods: {
    ...
    initSeniorBtns: function() {
      ...
      vm.seniorBtns = [
        ...
        { label: "撤消", str: "revokeUse" },
        { label: "删除", str: "delete" },
        { label: "停用", str: "stop" },
        { label: "启用", str: "start" },
        { label: "上架", str: "up" },
        { label: "下架", str: "down" },
        { label: "可见设置", str: "usable" },
        { label: "屏蔽设置", str: "blind" },
        { label: "计费规则", str: "rule" },
        { label: "申请使用", str: "applyUse" },
        { label: "再次申请", str: "applyUseAgain" },
        { label: "查看创建申请", str: "viewApplyRegist" },
        { label: "查看使用申请", str: "viewApplyUse" },
        { label: "查看原因", str: "viewReason" },
        { label: "发布", str: "release" }
      ];
    },
    //初始化排序参数
    initSort: function() {
