      ...
      <div class="servermode-div lanm-con">
        ...
        <div class="basicInformation">
          ...
          <div class="callInformation">
            <div>
              <div class="calldetail-right">
                <div class="callnum">{{click_num}}</div>
                <div class="calltitle">浏览量</div>
              </div>
              <div class="calldetail-center">
                <div class="callnum">{{user_num}}</div>
                <div class="calltitle">累计用户量</div>
              </div>
              <div class="calldetail">
                <div class="callnum">{{visit_num}}</div>
                <div class="calltitle">调用次数</div>
              </div>
            </div>
          </div>
            ...
            <ul class="modifyDeleteUl">
              <li v-for="item in btnData" class="modifyDeleteLi">
                <div>
                  ...
                  <span class="calltitle" @click="onClickLable(item.code,formData)">{{ item.label }}</span>
                </div>
              </li>
            </ul>
          </div> -->
        ...
        <div class="serviceInformation">
          ...
          <ul v-show="isup">
            ...
            <li>
              ...
              <div class="contentremark"><pre>{{service_remark}}</pre></div>
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      apiClient.post(
        "itsmApi/combination/findCombinationServiceDetail",
        param,
        function(data) {
          ...
          that.successUrl2 =
            ...
            apiClient.token;
          if (
            data.combination_data_config != undefined ||
            data.combination_data_config != ""
          ) {
            data.combination_data_config.block = JSON.parse(
              data.combination_data_config.block
            );
            data.combination_data_config.connects = JSON.parse(
              data.combination_data_config.connects
            );
    ...
    submitApply(param) {
      ...
      apiClient.post("itsmApi/atomicService/auditAtomic", myData, function(
        data
      ) {
        LoadingBoxService.close();
        if (data.status == 200) {
    ...
    getApprovalInfo() {
      ...
      apiClient.post("itsmApi/applyInfo/loadAuditFlowInfo", myData, function(
        data
      ) {
        if (data.status == 200) {
    ...
    loadChartByJSON(data) {
      ...
      for (var i = 0; i < unpack["block"].length; i++) {
        ...
        var html =
          '<div id="' +
          BlockId +
          '" class="nodeDiv" style="border: 1px #d5d5d5 solid;width: auto;height: auto;position: absolute;left: 40%;top: 50px;border-radius: 3px;background: rgb(255, 255, 255) !important">' +
          '<span class="editServerName" style="display: block;font-size: 12px;height:12px;line-height:12px;margin:10px;" title="' +
          BlockContent +
          '">' +
          BlockContent +
          "</span>" +
