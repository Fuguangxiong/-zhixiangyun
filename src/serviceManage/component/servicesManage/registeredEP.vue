...
export default {
  ...
  data() {
    return {
      breadcrumb: [{name: '服务管理', link:'/epsearch'}, {name: '我的注册', link: ''}],
      sourceType: "5", //已注册的服务
      searchKey: {}, //查询关键字
      selectSetting: [], //筛选项
  ...
  mounted() {
    this.getMenuAndBtns();  
    this.setSelectData(); 
    this.$parent.setActive('服务管理','我的注册');   
  },
  watch: {
    list (val, oldVal) {
      this.showBtn(val)
  ...
  methods: {
          ...
          for(var j = 0; i < menuList.length; j++){
              if(menuList[j].menuId == menuData.menuId){
                ...
                for(let index in this.btnArray){
                  if ((this.btnArray)[index] == 'sc_newService') {
    ...
    setSortSetting: function() {
      ...
      vm.sortSetting = [
        ...
        { label: "归属", paramName: "apptype", sortType: "desc" },
