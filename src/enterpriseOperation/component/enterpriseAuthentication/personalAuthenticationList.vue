<template class="public">
  <div class="personalAuthentication">
	...
	<div class="searchBoxpersonal">
	  用户名称: <input type="text" class="inputClass" placeholder="请输入申请人的姓名" v-model="content"><span @click="searchApply"><i class="iconfont icon-sousuo"></i></span>
	</div>
	<!--多选状态-->
		  ...
		  <div class="listCon">
			...
			<div class="datePickerclass">
			  ...
			  <transition name="fade">
				<button @click="comfirmTime" v-show="buttonShow">确认</button>
			  </transition>
			</div>
		  </div>
		</div>
...
//        that.approvalStatus = val
      ...
      goDetail (id) {
        this.$router.push({path: '/personalAuthenticationDetail', query: {id: id}})
      },
      // 获取列表数据
      getTableData () {
...
//          that.table.push(data.data)
      ...
      comfirmTime () {
        if (this.value11 == '' || this.value12 == '') {
          TipBoxService.open('请选择需要查询的时间', 2, 2)// 验证正面照
          return false
        }
        var that = this
//        that.buttonShow = false
