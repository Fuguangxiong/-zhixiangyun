	...
	var vue = new Vue({
		...
		methods: {
			...
			getUpdateInfo: function() {
				var that = this;
				//根据id查部门信息
				HttpService.post('enterprise/saveEnterprise', 'param=' + paramId, function(data) {
					console.info("ddddddddd=", data);
					if(data.status == '200') {
						that.basicInfoForm.id = data.data.id;
						that.basicInfoForm.userName = data.data.userName;
						that.basicInfoForm.userCard = data.data.userCard;
						that.basicInfoForm.idCardScanFront = data.data.idCardScanFront;
						that.basicInfoForm.idCardScanBack = data.data.idCardScanBack;
						that.basicInfoForm.enterpriseName = data.data.enterpriseName;
						that.basicInfoForm.blregistNum = data.data.blregistNum;
						that.basicInfoForm.blscanCopy = data.data.blscanCopy;
						that.basicInfoForm.legalDelegate = data.data.legalDelegate;
						that.basicInfoForm.legalDelegateId = data.data.legalDelegateId;
						that.basicInfoForm.companyAddress = data.data.companyAddress;
						that.basicInfoForm.province = data.data.province,//所在省份
						that.basicInfoForm.city = data.data.city,//所在城市
						that.basicInfoForm.detailedAddress = data.data.detailedAddress;
			...
			//获取数据
				tableLinkHandler(id) {
					...
					var that = this;
					//后台的接口
					HttpService.get('enterprise/searchEnterpriseInfo',param, function(data) {
						console.info("信息请求=", data);
						if(data.status == '200') {
							that.basicInfoForm.userId = data.data.list[0].USERID;
							that.basicInfoForm.userName = data.data.list[0].USERNAME;
							that.basicInfoForm.userCard = data.data.list[0].USER_CARD;
							...
							that.basicInfoForm.enterpriseName = data.data.list[0].ENTERPRISE_NAME;
							that.basicInfoForm.blregistNum = data.data.list[0].BLREGISTNUM;
							that.basicInfoForm.blscanCopy = HttpService.apiBaseUrl+data.data.list[0].BLSCANCOPY;
							that.basicInfoForm.blscanCopy2 = data.data.list[0].BLSCANCOPY;
							that.basicInfoForm.legalDelegate = data.data.list[0].LEGAL_DELEGATE;
							that.basicInfoForm.legalDelegateId = data.data.list[0].LEGAL_DELEGATE_ID;
							that.basicInfoForm.companyAddress = data.data.list[0].COMPANY_ADDRESS;//公司地址
							that.basicInfoForm.province = data.data.list[0].PROVINCE, //所在省份
								that.basicInfoForm.city = data.data.list[0].CITY, //所在城市
								that.basicInfoForm.detailedAddress = data.data.list[0].DETAILED_ADDRESS;
						}
					})
				},
			//保存基本信息
			saveBasicInfo: function() {
				...
				if(this.basicInfoForm.userName == '') {
					...
					if(!ToolService.regUserName(this.basicInfoForm.userName)) {
						...
						return false;
								...
								if(this.basicInfoForm.companyAddress==''){					
				...
				HttpService.post('enterprise/updateEnterpriseInfo', 'param=' + JSON.stringify(param), function(data) {
